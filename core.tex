...
\begin{figure}
	\begin{mathpar}
		...
		\inferrule{\Alloc(\Clos~\Env(\text{fv})\cdots N~E, H) = (P, H')}{\Eval(\backslash N. E, \Env, K), H \leadsto \Apply(K, P), H'}
	\end{mathpar}
...
\begin{figure}
	\begin{mathpar}
		...
		\inferrule{\Lookup(P, H) = \KApp_0~\Env~X~K \and \Lookup(V, H) = \Clos~\Env'~N~E \and \Alloc(\KApp_1~\Env'~N~E~K, H) = (P', H')}{\Apply(P, V), H \leadsto \Eval(X, \Env, P'), H'} \and
